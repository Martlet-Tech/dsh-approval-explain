...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * 「解释」按钮的 DeepSeek 家族配色类名。
     *
     * 用官方 static token 而非写死色值：
     *   --dsw-static-deepseek-50  = #EDF3FE（浅底）
     *   --dsw-static-deepseek-500 = #4176E6（文字与描边）
     * 跟随主题，暗色模式下也有对应取值。
     */
    const TINT_CLASS = 'dsh-approval-explain-action'

    /**
     * 注入配色样式表（幂等）。
     *
     * 必须用 `button.<class>` 这种「元素+类」选择器：Button 原语自身的
     * `.outline` / `.button` 都是单类选择器，元素+类的特异性更高，
     * 才能稳定压过它们的 `border:none` 与默认描边色，且不受样式表顺序影响。
     *
     * @returns 卸载时移除样式的清理函数。
     */
    function installTintStyle() {
      const tagId = 'dsh-approval-explain/action-tint.css'
      ...
      tag.dataset.plugin = 'dsh-approval-explain'
      tag.dataset.pluginCss = tagId
      tag.textContent = [
        `button.${TINT_CLASS}{`,
        'background:var(--dsw-static-deepseek-50);',
        'border:0.5px solid var(--dsw-static-deepseek-500);',
        'color:var(--dsw-static-deepseek-500);}',
        `button.${TINT_CLASS}:hover:not(:disabled){`,
        'background:var(--dsw-static-deepseek-100);',
        'border-color:var(--dsw-static-deepseek-500);',
        'color:var(--dsw-static-deepseek-500);}',
      ].join('')
    ...
    function ApprovalExplainCard(props) {
      ...
      const actionRow = createElement('div', { style: actionRowStyle }, [
        // 与「拒绝」按钮同族：outline + 默认 md（h36 / 14px / r18 / 描边），
        // 保证与原生两个决定按钮逐像素同高同宽同圆角。
        // 原为 variant:'ghost' + size:'sm'（h28 / 12px / r14 / 无边框），
        // 视觉上比原生按钮矮一档且没有描边，故改齐。
        // 配色改用 DeepSeek 家族色（见 installTintStyle），
        // 图标走原语自带的前导 icon 槽（16px 盒子 + 4px gap）。
        createElement(primitives.Button, {
          key: 'explain',
          variant: 'outline',
          className: TINT_CLASS,
    ...
    function apply(ctx) {
      ...
      ctx.effect(() => installTintStyle(), 'approval-explain: action tint style')
